    ...
    if request.method == "POST":
        ...
        with sqlite3.connect('database.db') as conn:
            try:
                ...
                cur.execute('Insert into orders (name, price, description, image, stock, categoryId) VALUES (?, ?, ?, ?, ?, ?)''', (name, price, description, imagename, stock, categoryId))
                conn.commit()
            except:
                msg="Error occured"
                conn.rollback()
...
def removeItem():
    ...
    with sqlite3.connect('database.db') as conn:
        try:
            ...
            cur = conn.cursor()
            cur.execute('DELETE FROM orders WHERE productID = ' + productId)
        except:
            conn.rollback()
            msg = "Error occured"
    conn.close()
...
def search_results():
        ...
        data = parse(data)
        return render_template('search.html', data=data, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems, categoryName=categoryName)
